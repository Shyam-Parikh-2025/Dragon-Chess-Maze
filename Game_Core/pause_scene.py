...
from Game_Core.ui import UI_MANAGER, Button
from Game_Core.menu_scenes import StartScreen
from Game_Core.game import Game

# ── LAYOUT ────────────────────────────────────────────────────────────────────
SX = 185
SY = 90
_TY = -int(SY * 1.5)  # Top row Y
_BY =  int(SY * 1.5)  # Bottom row Y
...
# The Elite 12 Layout: Linear straight-line branches radiating from the center!
SKILL_OFFSETS = {
    "root":            (      0,    0),

    # ── Sentinel (Chess) — top-left 
    "pawn_shield":     (  -SX,  _TY),
    "royal_guard":     (-SX*2,  _TY),
    "berserker_king":  (-SX*3,  _TY),

    # ── Guardian (Defense) — top-right
    "sturdy_mind":     (   SX,  _TY),
    "soft_landing":    ( SX*2,  _TY),
    "second_wind":     ( SX*3,  _TY),

    # ── Explorer (Utility) — bottom-left
    "swift_boots":     (  -SX,  _BY),
    "speedster":       (-SX*2,  _BY),
    "extra_spoils":    (-SX*3,  _BY),

    # ── Seer (Navigation) — bottom-right
    "beacon":          (   SX,  _BY),
    "boss_tracker":    ( SX*2,  _BY),
    "warp":            ( SX*3,  _BY),
}
 
BRANCH_LABELS = [
    ("-- SENTINEL --", -SX*2, _TY - 55, BRANCH_COLORS["crimson"]),
    ("-- GUARDIAN --",  SX*2, _TY - 55, BRANCH_COLORS["sapphire"]),
    ("-- EXPLORER --", -SX*2, _BY - 55, BRANCH_COLORS["emerald"]),
    ("-- SEER --",      SX*2, _BY - 55, BRANCH_COLORS["amber"]),
]
...
SKILL_DEFS = {
    ...
    "berserker_king":  ("BERSERKER KING",   "King jumps like Knight",     500, ["royal_guard"],                          "crimson"),
    # ── Guardian ──────────────────────────────────────────────────────────────
    "sturdy_mind":     ("STURDY MIND",      "Additional life",           200, ["root"],                                 "sapphire"),
    "soft_landing":    ("SOFT LANDING",     "Loss = respawn near portal",300, ["sturdy_mind"],                          "sapphire"),
    "second_wind":     ("SECOND WIND",      "Survive death once per run",500, ["soft_landing"],                         "sapphire"),
    # ── Explorer ──────────────────────────────────────────────────────────────
    "swift_boots":     ("SWIFT BOOTS",      "Move speed +15%",           200, ["root"],                                 "emerald"),
    "speedster":       ("SPEEDSTER",        "Hold Shift = 2x speed",     350, ["swift_boots"],                          "emerald"),
    "extra_spoils":    ("EXTRA SPOILS",     "+25% score per battle",     450, ["speedster"],                            "emerald"),
    # ── Seer ──────────────────────────────────────────────────────────────────
    "beacon":          ("BEACON",           "M=Map, Click=Waypoint",     200, ["root"],                                 "amber"),
    "boss_tracker":    ("BOSS TRACKER",     "Boss dot after 2 kills",    350, ["beacon"],                               "amber"),
    "warp":            ("WARP",             "Press 'T' to warp to boss", 600, ["boss_tracker"],                         "amber"),
}
...
class Skill:
    ...
    def draw(self, surface, selected, scroll_x, scroll_y):
        ...
        ns = Skill._font_name.render(self.display_name, True, fc) #type: ignore
        ds = Skill._font_desc.render(self.description,  True, dc) #type: ignore
...
class PauseScene(Scene):
    ...
    def quit(self):
        self.game.change_scene(StartScreen(self.game, "Continue Game"))
