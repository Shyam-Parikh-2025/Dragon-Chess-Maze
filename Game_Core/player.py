...
class Player:
    def __init__(self, start_pos, game, surface=None, radius=0.3):
    ...
    def log_msg(self, *args):
        if hasattr(self.game, 'terminal'):
            dev_mode = False
            if getattr(self.game, 'maze_scene', None) is not None:
                dev_mode = getattr(self.game.maze_scene, 'dev_mode', False)
            self.game.terminal.log_msg(dev_mode, *args)
        else:
            print(*args)           
    ...
    def update(self, keys, grid, delta_time = 0.016):
        ...
        if self.can_move:
            ...
            if delta_x != 0.0 and delta_z != 0.0:
                self.bob_time += delta_time * (speed * 120.0)
