from Chess_Engine.chess_engine import Chess, decode_move
from Chess_Engine.dragon import Dragon
...
class Battle:
    def __init__(self, surface, dragon_level, dragon_name, player, game, super_mode=0):
    ...
    def _calculate_move_thread(self, chess_engine_clone):
        self.calculated_move = self.dragon.get_move(chess_engine_clone, 
                                                    self.dragon_level, self.player.chess_difficulty)
    ...
    def player_lost(self):
        ...
        if self.player.second_wind and not self.player._second_wind_used:
            ...
            self.player.log_msg("Second Wind!")
            self.chess_engine = Chess(super_mode=self.super_mode)
        ...
        if self.player.kings_ransom and self.player.score >= 500:
            ...
            self.player.log_msg("King's Ransom paid - 500 pts spent to survive!")
            self.chess_engine = Chess(super_mode=self.super_mode)
            self.chess_engine.berserker_king = self.player.berserker_king
            self._apply_board_skills()
            self.game_over_processed = False
        ...
        self.player.playing_chess = False
        self.player.can_move = True
        self.player.log_msg("Lost a Life! You can do it!")
    ...
    def update_score(self, difficulty):
        ...
        if self.player.blitz_specialist and len(self.chess_engine.move_log) <= 40:
            score *= 2
            self.player.log_msg("Blitz Specialist! Double points!")
    ...
    def check_game_over(self, player):
        if self.chess_engine.is_checkmate():
            ...
            if player.peace_treaty:
                self.player.log_msg("Peace Treaty - stalemate ignored!")
                return False
            self.player.log_msg("Good Progress! Try again!")
