...
from Graphics.shaders.shaders_3D import VERTEX_SHADER_3D_TAMING_SCENE, FRAGMENT_SHADER_3D_TAMING_SCENE, VERTEX_SHADER_3D, FRAGMENT_SHADER_3D
from Graphics.shaders.shaders_2D import VERTEX_SHADER_2D, FRAGMENT_SHADER_2D
from Graphics.terminal import Terminal
from Game_Core.player import Player
import constants
import os
 
class Graphics3d:
    def __init__(self, screen, maze_grid):
        ...
        self.prog = self.ctx.program(
            vertex_shader=VERTEX_SHADER_3D_TAMING_SCENE,
            fragment_shader=FRAGMENT_SHADER_3D_TAMING_SCENE
        )

        self.maze_program = self.ctx.program(
            vertex_shader=VERTEX_SHADER_3D,
            fragment_shader=FRAGMENT_SHADER_3D
        )

        vbo_cube = self.ctx.buffer(self._cube_data())
        instance_data = self.get_instance_data(maze_grid)
        self.num_instances = len(instance_data)
        vbo_instances = self.ctx.buffer(instance_data)

        self.maze_vao = self.ctx.vertex_array(
            self.maze_program,
            [(vbo_cube, '3f', 'in_pos'),
             (vbo_instances, '3f 1f /i', 'in_instance_pos', 'in_instance_type')]
        )
    ...
    def refresh_instances(self, maze_grid):
        instance_data = self.get_instance_data(maze_grid)
        vbo_instances = self.ctx.buffer(instance_data)
    ...
    def update_view(self, player):
        bob_offset = math.sin(player.bob_time) * 0.03 if player.can_move else 0.0

        cam     = np.array([player.pos[0], 0.5 + bob_offset, player.pos[1]], dtype="float32")
        dir_cam = np.array([np.cos(player.angle_x), 0.0, np.sin(player.angle_x)], dtype="float32")
        up_vec  = np.array([0.0, 1.0, 0.0], dtype="float32")
        self.view    = matrix44.create_look_at(cam, cam + dir_cam, up_vec)
        self.prog["m_view"].write(self.view.astype("float32"))
    ...
    def render_maze(self, grid, wall_color, portal_color, player: Player):
        ...
        max_dist = 15.0 if player.clarity else 12.0
        portal_on = player.portal_tower

        self.maze_program['m_proj'].write(self.projection.astype("float32"))
        self.maze_program['m_view'].write(self.view.astype('float32'))

        self.maze_program['u_player_pos'].value = (player.pos[0], 0.5, player.pos[1])
        self.maze_program['u_wall_color'].value = wall_color
        self.maze_program['u_portal_color'].value = portal_color
        self.maze_program['u_max_dist'].value = max_dist
        ...
        for r in range(rows):
            for c in range(cols):
                ...
                dist = math.hypot(player.pos[0] - c, player.pos[1] - r)
                portal_enlarge = (portal_on and dist < 10.0)
                ...
                if val == 3: r_c, g_c, b_c = portal_color
                elif val == 4: r_c, g_c, b_c = 1.0, 0.0, 0.0
                else:          r_c, g_c, b_c = 0.5, 0.5, 0.5
 
                self.prog["u_color"].value = (r_c, g_c, b_c)
...
class Graphics2d:
    def __init__(self, surface, player, terminal, txt_size=36, big_txt_size=72):
    ...
    def load_dragon_imgs(self):
        ...
        try:
            for drag in ['dragon_normal.png', 'dragon_thinking.png']:
                ...
                image_folder = os.path.abspath(os.path.join(base_path, "..", "images"))
            ...
            self.player.log_msg("Dragon images file not found, shifting to writing")
    ...
    def draw_minimap(self, grid, player, wall_color: tuple, portal_color: tuple):
        ...
        # boss tracker
        if player.boss_tracker and player.dragons_beaten >= 2 and not player.boss_tracker_used:
            player.set_named_waypoint('B', grid.shape[0] - 3, grid.shape[1] - 3)
            player.boss_tracker_used = True
    ...
    def draw_terminal(self, dev_mode: bool = True):
        self.terminal.render(self.surface, dev_mode)